			...
			case I2cRequest::State::READY:
			{
				...
				switch (request->getType())
				{
					case I2cRequest::Type::RX:
					{
						...
						if (masterReceive(requestRx->getAddress(),
						                  requestRx->getBuffer(),
						                  requestRx->getSize()) != Status::OK)
						{
							requestRx->setState(I2cRequest::State::ERROR);
						}
					...
					{
						auto requestRx = reinterpret_cast<I2cRequestRx *>(request);
						if (memoryRead(requestRx->getAddress(),
					...
					case I2cRequest::Type::TX:
					{
						...
						if (masterTransmit(requestTx->getAddress(),
						                   requestTx->getData()) != Status::OK)
						{
							requestTx->setState(I2cRequest::State::ERROR);
						}
					...
					{
						auto requestTx = reinterpret_cast<I2cRequestTx *>(request);
						if (memoryWrite(requestTx->getAddress(),
...
auto I2cController::requestCompleted(uint8_t error) -> void
{
	if (!requestQueue.empty())
	{
		auto request = requestQueue.front();

		request->setState(error == 0U ? I2cRequest::State::COMPLETED : I2cRequest::State::ERROR);
	}
}
...
{
	switch (HAL_I2C_Master_Transmit_DMA(&hi2c1,
	                                    _address << 1,
	                                    _data->data(),
	                                    _data->size()))
	{
...
auto I2cController::memoryWrite(uint16_t _address,
                                ...
                                std::vector<uint8_t> *_data) -> Status
{
	switch (HAL_I2C_Mem_Write_DMA(&hi2c1,
	                              _address << 1,
	                              _registerAddress,
...
{
	switch (HAL_I2C_Master_Receive_DMA(&hi2c1,
	                                   _address << 1,
	                                   _buffer,
	                                   _size))
	{
...
auto I2cController::memoryRead(uint16_t _address,
                               ...
                               uint16_t _size) -> Status
{
	switch (HAL_I2C_Mem_Read_DMA(&hi2c1,
	                             _address << 1,
	                             _registerAddress,
	                             I2C_MEMADD_SIZE_8BIT,
	                             _buffer,
